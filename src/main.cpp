...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Protect: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Protect Signed Message:\n";
...
{
    // Protect: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 24 * 60 * 60; // Protect: 1 day
static const int64 nTargetSpacing = 42; // Protect: 1 minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Protect: This fixes an issue where a 51% attack can change difficulty at will.
...
{
        static const int64 BlocksTargetSpacing = 42; // Protect: 1 minute
        static const unsigned int TimeDaySeconds = 24 * 60 * 60; // Protect: 1 day
...
{
    // Protect: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb9, 0xde }; // Protect: increase each by adding 6 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ProtectMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ProtectMiner : ProcessBlock, block not accepted");
    }
